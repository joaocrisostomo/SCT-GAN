...
def collect_validation_results(
    ...
    for contract_idx in range(total_contracts):
        try:
            ...
            # Debug: Print shapes
            print(f"    Contract {contract_idx}: true_line_vulns shape: {true_line_vulns.shape}")
            
            # Get vulnerability predictions using the safe method
            try:
                analyzer_results = analyzer.detect_vulnerabilities_safe(
                    source_code, 
                    threshold=threshold
                )
            except Exception as e:
                print(f"    Error in detect_vulnerabilities_safe: {str(e)}")
